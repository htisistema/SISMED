...
PRIVATE mfornece,muf,mcodconta,mcod_forn,mcliente,mcpf,mcgc
	...
	exibi_prg(mprg)
        ...
        *** RESUMO DA CONSULTA ***
        ...
        ELSEIF tipo = 'P'
                ...
                IF mfor = NIL
                        cComm  := ccomm + " AND datpag >= "+sr_cdbvalue(mdata1)+" AND datpag <= "+sr_cdbvalue(mdata2)
                ENDIF
        ...
        ELSE
                ...
                IF mfor = NIL
                        cComm  := ccomm + " AND emissao >= "+sr_cdbvalue(mdata1)+" AND emissao <= "+sr_cdbvalue(mdata2)
                ENDIF
                IF ! EMPTY(mcod_forn)
                        ccomm := ccomm + " AND fornec = "+sr_cdbvalue(STRZERO(mcod_forn,4))
                ENDIF
        ...
        WHILE .T.
                ...
                point := ACHOICE(3,0,lba-5,cba,m_dup,,,point)
                DO CASE
                        CASE LASTKEY()=27
                                IF mdat1 <> NIL
                                        wvw_lclosewindow()
